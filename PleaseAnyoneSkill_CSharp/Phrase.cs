...
namespace PleaseGoogleHomeSkill_Csharp
{
    public static class Phrase
    {
        ...
        public static string ComposeAskSmartSpeakerText(string phrase)
        {
            ...
            try
            {
                var regResult = Regex.Match(phrase, "^(?<who>.+?)に(?<ask>.+)$");
                if (!regResult.Success)
                {
                    return "";
                }

                who = regResult.Groups["who"].Value;
                ask = regResult.Groups["ask"].Value;
            }
            catch (Exception e)
            {
                return "";
            ...
            //var startWithSkillNamePattern = @"^(ねえ)?.*?(google).*?(さん)?";
            ...
            //    }
            //}
            var trimmedAsk = Regex.Replace(ask, askPattern, "教えて");

            speechText += $"ねえ{who}、{trimmedAsk}";
